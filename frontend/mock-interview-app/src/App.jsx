...
/*
Hey bud. Here's your general boilerplate:

app.post('/submit-interview', async (req, res) => {
    const userResponses = req.body.responses; // User's answers from frontend
    const prompt = generatePrompt(userResponses); // Generate prompt dynamically
  
    const geminiResponse = await callGeminiAPI(prompt); // Call Gemini model via API
    const parsedData = JSON.parse(geminiResponse); // Parse JSON output from Gemini
  
    res.json(parsedData); // Send formatted data back to the frontend
  });
...
const App = () => {
  ...
  const handleSimulateInterview = async () => {
    const startMessage = { role: "user", parts: [{ text: "Tell me about yourself" }] };
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        <button onClick={handleSimulateInterview} className="simulate-button" disabled={isSimulating}>
          {" "}
          {isSimulating ? "  ... " : "Simulate my interview question"} {/* Button text */}
        </button>
      </div>
    </>
  );
